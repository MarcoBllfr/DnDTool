<script lang="ts">
    import { loadDataPlayer, loadMonsterData } from "$services/LocalStorage";
    ...
    let monsterTotalExp = $state(0);
    let expToGive = $state(0);
  ...
  if (monsterExp < playerThresholds.facile) {
    ...
    function loadAndCalculate() {
      ...
      if (monsterData) {
        monsterTotalExp = monsterData.totalExp;
        expToGive = monsterData.expSum;
        monsterCount = monsterData.listaMonster.reduce((total, monster) => total + monster.quantity, 0);
      }
